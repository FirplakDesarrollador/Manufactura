...
import { Search, X, Calendar, RefreshCw, Filter, FileText } from 'lucide-react'

interface DigitadoModuleProps {
    userEmail: string
    turno: string
...
export default function DigitadoModule({ userEmail, turno, usuarioNombre, plantaMuebles }: DigitadoModuleProps) {
    ...
    const loadData = React.useCallback(async () => {
        ...
        try {
            ...
            console.error('Error loading Digitado data:', error)
        } finally {
            setLoading(false)
        }
    }, [turno, plantaMuebles])
    ...
    const filteredOrdenes = useMemo(() => {
        ...
        return ordenes.filter((orden) => {
            ...
            // Only show orders with pieces ready for Digitado (WIP from Empaque)
            // Logic: empaque > 0
            const isPacked = (orden.empaque || 0) > 0

            return matchesSearch && matchesDate && isPacked
        })
                ...
                <div className="max-w-7xl mx-auto flex flex-col gap-4">
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 flex-1">
                            <div className="relative group">
                                <button 
                                    ...
                                    className={`p-2 rounded-lg text-white transition-colors duration-200 flex items-center gap-1 ${dateType === 'entrega' ? 'bg-cyan-600' : 'bg-purple-600'}`}
                                >
                                    <Calendar size={20} />
                            ...
                            <div className="relative flex-1">
                                ...
                                <input
                                    type="text"
                                    placeholder="Buscar en Digitado..."
                                    value={searchText}
                                    onChange={(e) => setSearchText(e.target.value)}
                                    className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 outline-none"
                                />
                            </div>
                        ...
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 md:pb-0 shrink-0">
                            ...
                            <MetricCard title="Digitado" value={sumDigitado} bgColor="bg-cyan-600" />
                            <MetricCard title="Digitado hoy" value={metricas?.digitado || 0} bgColor="bg-gray-400" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto bg-gray-50/50">
                <div className="max-w-7xl mx-auto p-4">
                    <div className="flex items-center justify-between mb-4">
                        <div className="text-cyan-600 text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                            <FileText size={14} />
                            ORDENES EN DIGITADO: {filteredOrdenes.length}
                        </div>
                    </div>
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4">
                            ...
                            <p className="text-gray-500 font-medium">Cargando órdenes de digitado...</p>
                        </div>
                    ) : filteredOrdenes.length === 0 ? (
                        <div className="text-center py-20">
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 inline-block">
                                <Search size={48} className="text-gray-200 mx-auto mb-4" />
                                <p className="text-gray-500 font-medium">No hay órdenes pendientes de digitado.</p>
                            </div>
                        </div>
                    ) : (
