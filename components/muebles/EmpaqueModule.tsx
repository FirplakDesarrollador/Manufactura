...
import { Search, X, Calendar, RefreshCw, Filter, Package, CheckSquare, Play } from 'lucide-react'

interface EmpaqueModuleProps {
    userEmail: string
    turno: string
...
export default function EmpaqueModule({ userEmail, turno, usuarioNombre, plantaMuebles, onStartTask }: EmpaqueModuleProps) {
    ...
    const loadData = useCallback(async (soft = false) => {
        ...
        try {
            ...
            console.error('Error loading Empaque data:', error)
        } finally {
            setLoading(false)
            setSyncing(false)
        }
    ...
    const filteredOrdenes = useMemo(() => {
        ...
        return ordenes.filter((orden) => {
            ...
            // Only show orders with pieces ready for Empaque (WIP from Inspeccion)
            // Logic: inspeccion > 0
            const isInspected = (orden.inspeccion || 0) > 0

            return matchesSearch && matchesDate && isInspected
        })
    ...
    const sumEmpaque = filteredOrdenes.reduce((sum, o) => sum + (o.empaque || 0), 0)
                ...
                <div className="max-w-7xl mx-auto flex flex-col gap-4">
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 flex-1">
                            <div className="relative group">
                                <button 
                                    ...
                                    className={`p-2 rounded-lg text-white transition-colors duration-200 flex items-center gap-1 ${dateType === 'entrega' ? 'bg-amber-600' : 'bg-purple-600'}`}
                            ...
                            <div className="relative flex-1">
                                ...
                                <input
                                    type="text"
                                    placeholder="Buscar en Empaque..."
                                    value={searchText}
                                    onChange={(e) => setSearchText(e.target.value)}
                                    className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none"
                                />
                            </div>
                        ...
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 md:pb-0 shrink-0">
                            ...
                            <MetricCard title="Empaque" value={sumEmpaque} bgColor="bg-amber-600" />
                            <MetricCard title="Empaque hoy" value={metricas?.empacados || 0} bgColor="bg-gray-400" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto bg-gray-50/50">
                <div className="max-w-7xl mx-auto p-4">
                    <div className="flex items-center justify-between mb-4">
                        <div className="text-amber-600 text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                            <Package size={14} />
                            ORDENES EN EMPAQUE: {filteredOrdenes.length}
                        </div>
                        {syncing && (
                            <div className="flex items-center gap-1.5 text-amber-600 text-[10px] font-bold uppercase tracking-widest animate-in fade-in duration-300">
                    ...
                    {selectedOrdenes.length > 0 && (
                        <div className="sticky top-2 z-20 mb-4 bg-white border border-amber-100 rounded-xl shadow-lg p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-400 font-bold uppercase">
                                        Total disponible: {selectedOrdenes.reduce((sum, item) => sum + (item.inspeccion || 0), 0)} piezas
                                    </p>
                                </div>
                            </div>
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4">
                            ...
                            <p className="text-gray-500 font-medium">Cargando órdenes de empaque...</p>
                        </div>
                    ) : filteredOrdenes.length === 0 ? (
                        <div className="text-center py-20">
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 inline-block">
                                <Search size={48} className="text-gray-200 mx-auto mb-4" />
                                <p className="text-gray-500 font-medium">No hay órdenes pendientes de empaque.</p>
                            </div>
                        </div>
                    ) : (
