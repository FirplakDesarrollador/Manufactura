...
import { getRegistrosParaEmpaque } from '@/lib/supabase/queries/fibra_empaque'
import RegistroEmpaqueCard from './RegistroEmpaqueCard'
import { Search, Eraser, ClipboardList, Loader2, Package } from 'lucide-react'

export default function EmpaqueFibraModule({ userEmail }: { userEmail: string }) {
    ...
    const loadRegistros = async () => {
        setLoading(true)
        try {
            const data = await getRegistrosParaEmpaque()
            setRegistros(data)
        } catch (error) {
            ...
            <div className="bg-white border-b border-slate-200 px-8 py-8 shadow-sm">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-slate-800 tracking-tight uppercase">Módulo Empaque</h1>
                            <p className="text-base text-slate-500 font-bold mt-1 uppercase tracking-widest opacity-80">
                                Despacho y salida de productos terminados.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white p-6 rounded-3xl border-2 border-slate-100 shadow-sm flex flex-col md:flex-row gap-6">
                        <div className="relative flex-1 group">
                            ...
                            <input
                                ...
                                className="w-full pl-14 pr-4 py-5 bg-slate-50 border-2 border-transparent rounded-2xl focus:bg-white focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 outline-none transition-all shadow-inner font-bold text-xl text-slate-800 placeholder:text-slate-400"
                            />
                        </div>
                        <button
                ...
                <div className="max-w-7xl mx-auto h-full">
                    {loading ? (
                        ...
                        <div className="bg-white rounded-[2rem] border-2 border-dashed border-slate-200 py-40 flex flex-col items-center justify-center text-center px-6 shadow-inner">
                            ...
                            <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Sin piezas para empacar</h3>
                            <p className="text-lg text-slate-500 max-w-sm mt-3 font-semibold">
                                {searchText
                                    ...
                                    : "No hay piezas en estado 'Acabado' pendientes por empacar."}
                            </p>
                        </div>
                    ) : (
