...
import { getRegistrosReparacion, ReparacionTab } from '@/lib/supabase/queries/fibra_reparacion'
import RegistroReparacionCard from './RegistroReparacionCard'
import { Search, Eraser, ClipboardList, Loader2, Wrench, Trash2, LayoutGrid, Info } from 'lucide-react'

export default function ReparacionFibraModule({ userEmail }: { userEmail: string }) {
    const [activeTab, setActiveTab] = useState<ReparacionTab>('Reparacion')
    ...
    const loadRegistros = async () => {
        setLoading(true)
        try {
            const data = await getRegistrosReparacion(activeTab)
            setRegistros(data)
        } catch (error) {
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-3xl font-black text-slate-800 tracking-tight uppercase">Módulo Reparación</h1>
                                <p className="text-base text-slate-500 font-bold mt-1 uppercase tracking-widest opacity-80">
                                    Gestión de piezas fuera de estándar.
                                </p>
                            </div>
                        </div>
                    </div>

                    <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shadow-inner overflow-x-auto scrollbar-hide">
                        <button
                            onClick={() => setActiveTab('Reparacion')}
                            className={`flex items-center gap-3 px-6 py-4 rounded-xl font-black transition-all ${activeTab === 'Reparacion'
                                ? 'bg-white text-orange-700 shadow-md scale-[1.02]'
                                : 'text-slate-500 hover:bg-white/50'
                                }`}
                        >
                            <Wrench size={24} />
                            <span className="uppercase tracking-widest whitespace-nowrap">Reparación</span>
                        </button>
                        <button
                            onClick={() => setActiveTab('Saldo')}
                            className={`flex items-center gap-3 px-6 py-4 rounded-xl font-black transition-all ${activeTab === 'Saldo'
                                ? 'bg-white text-orange-700 shadow-md scale-[1.02]'
                                : 'text-slate-500 hover:bg-white/50'
                                }`}
                        >
                            <Info size={24} />
                            <span className="uppercase tracking-widest whitespace-nowrap">Saldo</span>
                        </button>
                        <button
                            onClick={() => setActiveTab('Destruccion')}
                            className={`flex items-center gap-3 px-6 py-4 rounded-xl font-black transition-all ${activeTab === 'Destruccion'
                                ? 'bg-white text-red-700 shadow-md scale-[1.02]'
                                : 'text-slate-500 hover:bg-white/50'
                                }`}
                        >
                            <Trash2 size={24} />
                            <span className="uppercase tracking-widest whitespace-nowrap">Destrucción</span>
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="bg-white p-6 rounded-3xl border-2 border-slate-100 shadow-sm flex flex-col md:flex-row gap-6">
                        <div className="relative flex-1 group">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar sku, descripción, OF, pedido, molde..."
                                value={searchText}
                                onChange={(e) => setSearchText(e.target.value)}
                                className="w-full pl-14 pr-4 py-5 bg-slate-50 border-2 border-transparent rounded-2xl focus:bg-white focus:ring-2 focus:ring-orange-600 focus:border-orange-600 outline-none transition-all shadow-inner font-bold text-xl text-slate-800 placeholder:text-slate-400"
                            />
                        </div>
                        <button
