...
export default async function PuestaDetallePage({ params }: { params: Promise<{ planta: string, id: string }> | { planta: string, id: string } }) {
  try {
    const resolvedParams = params && typeof (params as any).then === 'function' ? await params : (params as any) || {};
    const planta = resolvedParams.planta || '';
    ...
    (det || []).forEach(row => {
      ...
      if (row.fecha_revision) {
        const fStr = String(row.fecha_revision);
        const parts = fStr.split('-');
        const part = parts.length > 2 ? parts[2] : null;
    ...
    const fecha  = enc.creado_en ? String(enc.creado_en).split('T')[0] : new Date().toISOString().split('T')[0];
          ...
          <div className="hdt-container">
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ width: '28%', padding: 0 }}>
                     <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                       <tbody>
                         <tr style={{ borderBottom: '1pt solid #000' }}><td style={{ border: 'none', padding: '2px 6px' }}><span className="hdt-label">CÓDIGO:</span><span className="hdt-value">SMA-{id ? String(id).substring(0,6).toUpperCase() : ''}</span></td></tr>
            ...
            <table className="hdt-table" style={{ marginTop: '-1pt' }}>
              <tbody>
                ...
                <tr>
                  <td style={{ width: '25%' }}><div className="hdt-label">CREADO POR</div><div className="hdt-value">{clean(enc['Creado por']) || clean(enc.creado_por) || 'Jakeline Chaverra'}</div></td>
                  <td style={{ width: '25%' }}><div className="hdt-label">MODIFICADO POR</div><div className="hdt-value">{clean(enc['Modificado por']) || clean(enc.modificado_por) || 'N/A'}</div></td>
    ...
    console.error('Error rendering PuestaDetallePage:', err);
    return (
      <div style={{ padding: '4rem', textAlign: 'center', color: '#D13438' }}>
        <h2>Error al cargar el detalle</h2>
        <pre style={{ margin: '1rem 0', color: '#667A85', textAlign: 'left', background: '#f5f5f5', padding: '1rem', borderRadius: '8px', overflow: 'auto' }}>
